        ...
        <main id="main-content" class="flex-1 p-4 md:p-6 transition-all duration-300 ease-in-out">
            ...
            <div class="bg-white rounded-lg shadow-md p-4 md:p-6 mb-6">
                ...
                <div class="flex flex-wrap items-center gap-x-4 gap-y-2 mb-6 text-sm">
                    ...
                    <div class="flex items-center gap-2">
                        <div class="w-4 h-4 rounded-full" style="background-color: #FFD700;" title="Incomplete"></div>
                ...
                @if ($dailyAttendances->isEmpty())
                    <p class="text-gray-600 text-center text-sm py-4">
                        @if (Request::filled('selected_date'))
                            Tidak ada data absensi untuk tanggal {{ \Carbon\Carbon::parse(Request::get('selected_date'))->translatedFormat('d F Y') }}.
                        @else
                ...
                @else
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                        @foreach ($dailyAttendances as $attendance)
                            <div class="bg-white rounded-lg shadow-sm p-4 flex items-start justify-between border border-gray-200 hover:shadow-md transition-shadow duration-200">
                                <div class="flex items-start">
                                    <div class="w-9 h-9 bg-gray-100 rounded-full flex items-center justify-center flex-shrink-0 mr-3">
                                    ...
                                    <div class="flex flex-col">
                                        ...
                                        <div class="text-xs text-gray-700">
                                            <p>Check-In: <span class="font-medium">{{ $attendance->check_in ? $attendance->check_in->tz('Asia/Makassar')->format('H:i') : '--.--' }}</span></p>
                                            <p>Check-Out: <span class="font-medium">{{ $attendance->check_out ? $attendance->check_out->tz('Asia/Makassar')->format('H:i') : '--.--' }}</span></p>
                                        </div>
                                        @if ($attendance->activity_title)
                                            <p class="text-xs text-gray-600 mt-2"><strong>Aktivitas:</strong> {{ $attendance->activity_title }}</p>
                                        @endif
                                        @if ($attendance->activity_description)
                                            <p class="text-xs text-gray-500 leading-tight mt-1">{{ $attendance->activity_description }}</p>
                                        @endif
                                    </div>
                                </div>
