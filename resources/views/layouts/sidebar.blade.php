...
<button id="mobile-menu-toggle" class="md:hidden fixed bottom-4 right-4 p-3 z-50 bg-[#2C3E50] text-white rounded-full shadow-lg hover:bg-[#3C5A6D] focus:outline-none">
    ...
    <div>
        ...
        <nav class="mt-6 px-4 space-y-2 pb-4">
            ...
            @can('access-user-pages')
                <a href="{{ route('dashboard') }}" class="flex items-center gap-3 px-4 py-2 rounded-xl transition duration-150 {{ request()->routeIs('dashboard') ? 'bg-[#FFD100] text-black' : 'hover:bg-[#3C5A6D]' }}">
                    <i class="fa-solid fa-house"></i> <span class="nav-text">Dasboard</span>
                </a>

                <div class="relative">
                    <button id="btn-attendance" type="button" class="dropdown-toggle flex items-center justify-between w-full px-4 py-2 rounded-xl transition duration-150 cursor-pointer {{ request()->routeIs('attendance.*') ? 'bg-[#FFD100] text-black' : 'hover:bg-[#3C5A6D]' }}">
                    ...
                    <div id="attendanceDropdown" class="dropdown-menu hidden mt-2 space-y-1 rounded-xl bg-[#34495E]">
                        <a href="{{ route('attendance.history') }}" class="block px-6 py-2 text-sm {{ request()->routeIs('attendance.history') ? 'bg-[#2C3E50] text-[#FFD100]' : 'hover:bg-[#2C3E50] hover:text-[#FFD100]' }} rounded">Riwayat</a>
                        <a href="{{ route('attendance.my') }}" class="block px-6 py-2 text-sm {{ request()->routeIs('attendance.my') ? 'bg-[#2C3E50] text-[#FFD100]' : 'hover:bg-[#2C3E50] hover:text-[#FFD100]' }} rounded">Absensi Saya</a>
                    </div>
                </div>

                <div class="relative">
                    <button id="btn-approval" type="button" class="dropdown-toggle flex items-center justify-between w-full px-4 py-2 rounded-xl transition duration-150 cursor-pointer {{ request()->routeIs('user.approval.requests') ? 'bg-[#FFD100] text-black' : 'hover:bg-[#3C5A6D]' }}">
                        <div class="flex items-center gap-3">
                    ...
                    <div id="approvalDropdown" class="dropdown-menu hidden mt-2 space-y-1 rounded-xl bg-[#34495E]">
                        <a href="{{ route('user.approval.requests') }}" class="block px-6 py-2 text-sm {{ request()->routeIs('user.approval.requests') ? 'bg-[#2C3E50] text-[#FFD100]' : 'hover:bg-[#2C3E50] hover:text-[#FFD100]' }} rounded">Persetujuan Absensi</a>
                    </div>
                </div>
            @endcan
            ...
            @can('access-admin-pages')
                ...
                @can('access-superadmin-pages')
                    <a href="{{ route('superadmin.dashboard') }}" class="flex items-center gap-3 px-4 py-2 rounded-xl transition duration-150 {{ request()->routeIs('superadmin.dashboard') ? 'bg-[#FFD100] text-black' : 'hover:bg-[#3C5A6D]' }}">
                ...
                <a href="{{ route('admin.dashboard') }}" class="flex items-center gap-3 px-4 py-2 rounded-xl transition duration-150 {{ request()->routeIs('admin.dashboard') ? 'bg-[#FFD100] text-black' : 'hover:bg-[#3C5A6D]' }}">
                    <i class="fa-solid fa-user-gear"></i> <span class="nav-text">Admin Dashboard</span>
                </a>

                {{-- Dropdown Monitoring --}}
                <div class="relative">
                    <button id="btn-monitoring" type="button" class="dropdown-toggle flex items-center justify-between w-full px-4 py-2 rounded-xl transition duration-150 cursor-pointer">
                        <div class="flex items-center gap-3">
                    ...
                    <div id="monitoringDropdown" class="dropdown-menu hidden mt-2 space-y-1 rounded-xl bg-[#34495E]">
                        <a href="#" class="block px-6 py-2 text-sm hover:bg-[#2C3E50] hover:text-[#FFD100] rounded">LOG</a>
                        <a href="#" class="block px-6 py-2 text-sm hover:bg-[#2C3E50] hover:text-[#FFD100] rounded">Users</a>
                    </div>
                </div>
            ...
            <form method="POST" action="{{ route('logout') }}">
                @csrf
                <button type="submit" class="w-full flex items-center gap-3 px-4 py-2 rounded-xl text-sm transition duration-150 hover:bg-[#3C5A6D]">
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        const setSidebarState = (isOpen) => {
            ...
            if (isDesktop) {
                ...
                document.querySelectorAll('.arrow-icon').forEach(icon => icon.classList.toggle('hidden', !isOpen));
                if (!isOpen) {
                    document.querySelectorAll('.dropdown-menu').forEach(menu => menu.classList.add('hidden'));
                    document.querySelectorAll('.arrow-icon').forEach(arrow => arrow.classList.remove('rotate-180'));
                }
            } else {
        ...
        mobileMenuToggle.addEventListener('click', () => setSidebarState(sidebar.classList.contains('-translate-x-full')));
...
<style>
    ...
    #sidebar::-webkit-scrollbar-thumb { background-color: #555; border-radius: 4px; border: 2px solid #2C3E50; }
    #sidebar.md\:w-20 .arrow-icon,
    #sidebar.md\:w-20 .nav-text,
    #sidebar.md\:w-20 #pln-text { display: none; }
    #sidebar.md\:w-20 #pln-logo-text,
    #sidebar.md\:w-20 nav a,
    #sidebar.md\:w-20 nav button { justify-content: center; }
</style>
