...
<div id="sidebar-overlay"
    class="hidden fixed inset-0 bg-black bg-opacity-60 z-30 md:hidden transition-opacity duration-300 ease-in-out opacity-0">
</div>
...
<aside id="sidebar"
    ...
    <div class="p-3 border-t border-gray-700">
        <button onclick="window.location.href='{{ route('profile.edit') }}'"
            class="w-full flex items-center gap-3 p-2 rounded-lg transition duration-150 hover:bg-[#3C5A6D] text-left">
            <img class="h-10 w-10 rounded-full object-cover"
                src="{{ optional(Auth::user())->profile_photo_path ? (Str::startsWith(optional(Auth::user())->profile_photo_path, 'profile_photos/') ? asset(optional(Auth::user())->profile_photo_path) : asset('storage/' . optional(Auth::user())->profile_photo_path)) : asset('profile_photos/avatar_1 (1).jpg') }}"
                alt="Foto Profil">
            <div class="nav-text transition-opacity duration-200">
                <p class="text-sm font-semibold text-white leading-4">
                    {{ Str::limit(optional(Auth::user())->name, 15) }}</p>
                <p class="text-xs text-gray-400">
                    {{ optional(Auth::user())->role ? Str::ucfirst(optional(Auth::user())->role) : 'User' }}</p>
            </div>
        </button>
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        if (desktopSidebarToggle) {
            ...
            const setDesktopSidebarState = (isOpen) => {
                ...
                sidebar.classList.toggle('md:w-60', isOpen);
        ...
        document.addEventListener('keydown', (e) => {
            if (e.key === 'Escape' && !sidebar.classList.contains('-translate-x-full') && window
                .innerWidth < 768) {
                toggleMobileSidebar();
            }
        });
